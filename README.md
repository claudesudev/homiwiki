...
### ⛔ 본문 텍스트: 줄표(em-dash, U+2014) 절대 금지 **[강제]**
웹에 노출되는 모든 텍스트(`content`, `title`, `subtitle`, `infobox`, `toc`, `related`)에
em-dash(U+2014) 를 **절대 사용하지 않는다.** AI 생성 문체의 대표 신호라 위키 신뢰도를 떨어뜨린다.
- 제목·정의형 `A[em-dash]B` 형태 → `A: B` (콜론)
- 문장 중간 삽입 → 쉼표(`,`) 또는 괄호(`( )`)
- 병렬 나열 → 가운뎃점(`·`)
- **강제 수단:** `scripts/verify.py` 가 검출 시 **[에러]로 검증 실패(exit 1)** 처리. 호버 금지와 동급 규약.
- 예외: 줄표 문자 자체를 예시로 *언급*하는 1건의 괄호 표기만 검토 후 허용.
